...
async function rememberPendingModmailConversation(
  context: RedisContext,
  subredditId: string,
  conversationId: string,
  verificationId: string
...
async function archivePendingVerificationModmailReply(
  context: Devvit.Context,
...
async function getAutoArchivePendingModmailEnabled(
  context: Pick<Devvit.Context, 'settings'>
): Promise<boolean> {
...
async function resolveModmailParticipantReplyIdentity(
  context: Devvit.Context,
  event: PendingModmailReplyEvent,
  conversationId: string
...
function getMostRecentModmailMessage(
  messages: Record<
    string,
    {
      id?: string;
...
function normalizeModmailPayloadToken(value: string | null | undefined): string {
  return String(value ?? '')
    .trim()
    .toLowerCase()
    .replace(/[\s-]+/g, '_');
...
function normalizeTriggerSubredditId(context: { subredditId?: string | null }, value: string | null | undefined): string {
...
async function findPendingRecordForModmailReply(
  context: RedisContext,
  subredditId: string,
  username: string,
